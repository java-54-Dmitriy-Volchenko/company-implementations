...
public class CompanyMapsImpl implements Company, Persistable {
    ...
    public void save(String filePathStr) {
        try (PrintWriter writer = new PrintWriter(Files.newBufferedWriter(Path.of(filePathStr)))) {
    ...
    public void restore(String filePathStr) {
        try (BufferedReader reader = Files.newBufferedReader(Path.of(filePathStr))) {
            reader.lines().forEach(line -> {
                JSONObject jsonObject = new JSONObject(line);
                String className = jsonObject.getString("className");
                try {
                    Employee employee = (Employee) Class.forName(className)
                            .getConstructor().newInstance();
                    employee.setObject(line);
                    this.addEmployee(employee);
                } catch (Exception e) {
                    throw new RuntimeException("Error restoring", e);
                }
            });
        } catch (IOException e) {
            throw new RuntimeException("Error restoring", e);
